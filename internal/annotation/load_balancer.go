...
const (
	// LBID is the ID assigned to the Hivelocity Cloud Load Balancer by the
	// backend. Read-only.
	...
	// the Hivelocity Cloud API console.
	LBName Name = "load-balancer.hetzner.cloud/name"

	// LBDisablePublicNetwork disables the public network of the Hivelocity Cloud
	...
	//
	// This annotation is exclusively for Hivelocity internal testing purposes.
	...
	// LBSvcHTTPStickySessions enables the sticky sessions feature of Hivelocity
...
func isHTTP(s hvlb.LoadBalancerService) bool {
	return s.Protocol == hvlb.LoadBalancerServiceProtocolHTTP
}

func isHTTPHealthCheck(s hvlb.LoadBalancerService) bool {
	return s.HealthCheck.HTTP != nil && s.HealthCheck.Protocol == hvlb.LoadBalancerServiceProtocolHTTP
}

func isHTTPS(s hvlb.LoadBalancerService) bool {
	return s.Protocol == hvlb.LoadBalancerServiceProtocolHTTPS
}

func isHTTPSHealthCheck(s hvlb.LoadBalancerService) bool {
	return s.HealthCheck.HTTP != nil && s.HealthCheck.Protocol == hvlb.LoadBalancerServiceProtocolHTTPS
}
