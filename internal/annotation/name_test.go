...
func TestName_AddToService(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "set hvlb.LoadBalancerServiceProtocol",
			value:    hvlb.LoadBalancerServiceProtocolTCP,
			expected: map[string]string{string(ann): string(hvlb.LoadBalancerServiceProtocolTCP)},
		},
		{
			name:     "set []*hv.Certificate",
			value:    []*hv.Certificate{{ID: 1}, {ID: 2}},
			expected: map[string]string{string(ann): "1,2"},
		},
		{
			name:     "set []*hv.Certificate by name",
			value:    []*hv.Certificate{{Name: "cert-1"}, {Name: "cert-2"}},
	...
	tests := []typedAccessorTest{
		{
			name: "value set",
			svcAnnotations: map[annotation.Name]interface{}{
				ann: hvlb.LoadBalancerServiceProtocolHTTP,
			},
			expected: hvlb.LoadBalancerServiceProtocolHTTP,
		},
		{
			name: "value not set",
			err:  annotation.ErrNotSet,
	...
	tests := []typedAccessorTest{
		{
			name: "value set",
			svcAnnotations: map[annotation.Name]interface{}{
				ann: hvlb.LoadBalancerAlgorithmTypeLeastConnections,
			},
			expected: hvlb.LoadBalancerAlgorithmTypeLeastConnections,
		},
		{
			name: "value not set",
			err:  annotation.ErrNotSet,
	...
	tests := []typedAccessorTest{
		{
			name: "value set",
			svcAnnotations: map[annotation.Name]interface{}{
				ann: hv.NetworkZoneEUCentral,
			},
			expected: hv.NetworkZoneEUCentral,
		},
		{
			name: "value not set",
			err:  annotation.ErrNotSet,
	...
	tests := []typedAccessorTest{
		{
			name: "ids set",
			svcAnnotations: map[annotation.Name]interface{}{
				ann: []*hv.Certificate{{ID: 3}, {ID: 5}},
			},
			expected: []*hv.Certificate{{ID: 3}, {ID: 5}},
		},
		{
			name: "names set",
			svcAnnotations: map[annotation.Name]interface{}{
				ann: []*hv.Certificate{{Name: "cert-1"}, {Name: "cert-2"}},
			},
			expected: []*hv.Certificate{{Name: "cert-1"}, {Name: "cert-2"}},
		},
		{
			name: "value not set",
			err:  annotation.ErrNotSet,
	...
	tests := []typedAccessorTest{
		{
			...
			svcAnnotations: map[annotation.Name]interface{}{
				ann: hv.CertificateTypeUploaded,
			},
			expected: hv.CertificateTypeUploaded,
		},
		{
			name: "managed certificate",
			svcAnnotations: map[annotation.Name]interface{}{
				ann: hv.CertificateTypeManaged,
			},
			expected: hv.CertificateTypeManaged,
		},
		{
